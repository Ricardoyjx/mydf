...
class MemoryRunStore(RunStore):
    ...
    async def list_pending(self, *, before=None):
        ...
        results = [
            r
            for r in self._runs.values()
            if r["status"] == "pending" and r["created_at"] <= now
        ]
    ...
    async def list_inflight(self, *, before=None):
        ...
        results = [
            r
            for r in self._runs.values()
            if r["status"] in ("pending", "running") and r["created_at"] <= now
        ]
        results.sort(key=lambda r: r["created_at"])
        return results
    ...
    async def aggregate_tokens_by_thread(
        ...
        statuses = (
            ("success", "error", "running") if include_active else ("success", "error")
        )
        completed = [
            r
            for r in self._runs.values()
            if r["thread_id"] == thread_id and r.get("status") in statuses
        ]
        ...
        return {
            ...
            "total_input_tokens": sum(
                r.get("total_input_tokens", 0) for r in completed
            ),
            "total_output_tokens": sum(
                r.get("total_output_tokens", 0) for r in completed
            ),
